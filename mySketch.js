...
function setup() {
	...
	player = {
		...
		move: function () {
			...
			// 	 }
			if (this.dead) {
				home = 1; this.dead = false; this.y = height - 100; placeOY = round(placeOY / 40); placeO = round(placeO / 40); player.xR = 0;
				this.yVel = 0;
		...
		show: function () {
			...
			// 	this.yVel-=0
			// 	 }
			if (this.air) { this.rot += 8 } else {
				if (this.rot > round(this.rot / 90) * 90 + 40) { this.rot -= 38 } else {
	...
	if (home == 0) {
		...
		if (-player.y + height - 100 > height - 100) {
			if (camY > -player.y + height - 100 + 3) { camY -= 3 } else {
				if (camY < -player.y + height - 100 - 3) { camY += 3 } else { camY = -player.y + height - 100 }
			}
		} else { if (camY > -player.y + height - 100 + 3) { camY -= player.yVel + 30 } }
		translate(0, camY)
		background(23, 23, 70)
		ground()
		player.air = true
